"""Kilo quota provider.

Runs `kilo stats` and parses the output.
"""
from __future__ import annotations

import os
import re
import subprocess
from datetime import datetime

from ai_quota.cache import read_cache as _read
from ai_quota.cache import read_cache_updated as _read_updated
from ai_quota.cache import write_cache as _write

CACHE_FILE = os.environ.get("KILO_USAGE_CACHE", "/tmp/kilo-usage.cache")


def parse_usage(raw: str) -> list[dict]:
    """Parse `kilo stats` output into a list of quota entries."""
    entry = {}

    # Strip box-drawing characters for easier regex
    clean = re.sub(r"[┌─┐│├┤┘└┴┬┼█┃┏┓┗┛┣┫┳┻╋━]", " ", raw)

    # Overview
    m = re.search(r"Sessions\s+(\d+)", clean)
    if m:
        entry["sessions"] = int(m.group(1))
    m = re.search(r"Messages\s+(\d+)", clean)
    if m:
        entry["messages"] = int(m.group(1))
    m = re.search(r"Days\s+(\d+)", clean)
    if m:
        entry["days"] = int(m.group(1))

    # Cost & Tokens
    m = re.search(r"Total Cost\s+(\$[\d\.]+)", clean)
    if m:
        entry["total_cost"] = m.group(1)
    m = re.search(r"Avg Cost/Day\s+(\$[\d\.]+)", clean)
    if m:
        entry["avg_cost_day"] = m.group(1)

    m = re.search(r"Input\s+([\d\.]+[KMB]?)", clean)
    if m:
        entry["input_tokens"] = m.group(1)
    m = re.search(r"Output\s+([\d\.]+[KMB]?)", clean)
    if m:
        entry["output_tokens"] = m.group(1)

    # Tool Usage
    tools = []
    # Tool lines look like: " bash                 13 (34.2%) " after cleaning
    tool_matches = re.finditer(r"^\s*([\w\.-]+)\s+\d+\s+\(\s*([\d\.]+)%\)", clean, re.MULTILINE)
    for m in tool_matches:
        tools.append({
            "tool": m.group(1),
            "percent": float(m.group(2))
        })
    if tools:
        entry["tools"] = tools

    if not entry:
        return []

    return [entry]


def fetch_live() -> list[dict]:
    """Run `kilo stats` and return parsed entries."""
    try:
        # kilo stats might use colors/ANSI
        result = subprocess.run(["kilo", "stats"], capture_output=True, text=True, check=True)
        # Strip ANSI escape codes
        raw = re.sub(r'\x1B(?:[@-Z\\-_]|\[[0-?]*[ -/]*[@-~])', '', result.stdout)
        return parse_usage(raw)
    except Exception:
        return []


def read_cache() -> list[dict]:
    return _read(CACHE_FILE)


def read_cache_last_checked() -> float | None:
    return _read_updated(CACHE_FILE)


def write_cache(entries: list[dict]) -> None:
    _write(CACHE_FILE, entries)


def fmt_short(entries: list[dict]) -> str:
    if not entries:
        return "kilo: no data"
    e = entries[0]
    return f"Kilo: {e.get('total_cost', '$0.00')} | {e.get('input_tokens', '0')} in | {e.get('output_tokens', '0')} out"


def fmt_slack(entries: list[dict]) -> str:
    if not entries:
        return ":warning: No Kilo usage data found."
    e = entries[0]
    lines = ["*Kilo Usage*"]
    lines.append(f"• Total Cost: `{e.get('total_cost', '$0.00')}`")
    lines.append(f"• Tokens: `{e.get('input_tokens', '0')} In` / `{e.get('output_tokens', '0')} Out`")
    lines.append(f"• Activity: `{e.get('sessions', 0)} Sessions` / `{e.get('messages', 0)} Messages`")

    if e.get("tools"):
        tool_strs = [f"{t['tool']} ({t['percent']}%)" for t in e["tools"][:3]]
        lines.append(f"• Top Tools: {', '.join(tool_strs)}")

    return "\n".join(lines)
